...
readWallet(filename, function (err, keys) {
    ...
    var address_bulks = _.toArray(_.groupBy(address_list, function (addr, idx) {
        return Math.floor(idx / GROUP_LENGTH);
    }));
    ...
    Fiber(function () {
        ...
        _.each(address_bulks, function (address_list) {
            ...
            Fiber(function () {
                var url = _.reduce(address_list, function (memo, addr) {
                    return memo + addr + "|";
                }, "http://blockchain.info/multiaddr?active=");
                ...
                http.get(url, function (res) {
                    ...
                    res.on('end', function () {
                        ...
                        total += _.reduce(obj.addresses, function (memo, addr_json) {
                            return memo + addr_json.final_balance;
                        }, 0);
                        inner_future.return();
                    });
                });
...
function convertToAddress(key) {
    ...
    // Convert to Base58
    var address_array = [];
    _.each(address_buffer, function (b) {
        address_array.push(b)
    });
